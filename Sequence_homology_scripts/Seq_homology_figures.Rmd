...
# # load the old excel file -- NOT USED
# old_peptides <- read_xlsx("./MIPSA_pairwise_analysis_llf_lec.xlsx",
#                      sheet = "SupTable3.Replicated_FDR", col_names = FALSE)
# old_peptides <- old_peptides[-1 ,1:7]
# colnames(old_peptides) <- old_peptides[1,]
# old_peptides <- old_peptides[-1,]
...
```{r}
#4 peptides total
alignment_dir <- "./ANKRD16_CMV/water_alignments_assoc/"
files <- list.files(alignment_dir, pattern = "\\.txt$", full.names = TRUE)
alignment_summary_assoc_ANKRD16 <- do.call(rbind, lapply(files, extract_water_alignment_info))

alignment_dir <- "./ANKRD16_CMV/water_alignments_randomized_null_assoc/"
files <- list.files(alignment_dir, pattern = "\\.txt$", full.names = TRUE)
alignment_summary_assoc_ANKRD16_null <- do.call(rbind, lapply(files, extract_water_alignment_info))
plotNullvsObs(alignment_summary_assoc_ANKRD16, alignment_summary_assoc_ANKRD16_null,
              title = "EBV peptide alignments against ANKRD16")
```


# SLC30A4 vc. CMV
```{r}
# 131 peptides in total
...
prop_in_null <- function(null, obs, name){
  ...
  prop <- cbind(alignment = name, 
                count_95_null = ct,
                num_pep_associated = num_pep_associated,
                prop_95_null = prop,
...
prop_in_null( alignment_summary_assoc_IQCB1_null, alignment_summary_assoc_IQCB1, "CMV_IQCB1"),
prop_in_null( alignment_summary_assoc_HEXIM2_null, alignment_summary_assoc_HEXIM2, "CMV_HEXIM2"),
prop_in_null(alignment_summary_assoc_ANKRD16_null, alignment_summary_assoc_ANKRD16, "CMV_ANKRD"),
...
prop_in_null( alignment_summary_assoc_PSMB6_null, alignment_summary_assoc_PSMB6, "EBV_PSMB6"),
...
alignment_summaries_all_peptides <- do.call(rbind, lapply(list(                 
                                                              alignment_summary_assoc_CFHR1,
                                                               alignment_summary_assoc_DNAJC12,
                                                               alignment_summary_assoc_IQCB1,
                                                               alignment_summary_assoc_HEXIM2,
                                                              alignment_summary_assoc_ANKRD16,
...
write.csv(alignment_summaries_all_peptides, "./alignment_summaries_all_peptides_final_260713.csv")
alignment_summaries_all_peptides
```

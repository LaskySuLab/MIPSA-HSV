...
option_list <- list(
  ...
  make_option("--out_dir",      type="character", default="results1/Network_nonstrict", help="Output dir")
)
...
sig_ab$gene_symbol       <- gsub("ZNF559-ZNF177,ZNF177", "ZNF177",  sig_ab$gene_symbol)
...
run_network_analysis <- function(target_virus, outcome_type, human_dx, virus_dx, sig_ab_full) {
  ...
  if(nrow(dis_subset) > 0) {
    dis_df <- dis_subset %>% inner_join(dx_lookup, by = c("name" = "Disease")) %>%

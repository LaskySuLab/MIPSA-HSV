...
option_list <- list(
  ...
  make_option("--out_dir", type="character", default="results1/Figure4", help="Output dir")
)
...
auc_tab_df.lec1$Gene <- gsub("ZNF559-ZNF177,ZNF177", "ZNF177", auc_tab_df.lec1$Gene)
...
ab_tbl = subset(auc_tab_df.lec1[,c('Antibody', 'Gene', 'Virus','AUC')], auc_tab_df.lec1$AUC > 0.85) #19
...
auc_tab_df.llf1$Gene <- gsub("ZNF559-ZNF177,ZNF177", "ZNF177", auc_tab_df.llf1$Gene)
...
for (ii in c(1:9)) {
  ...
  subgroup1$Gene <- gsub("ZNF559-ZNF177,ZNF177", "ZNF177",  subgroup1$Gene)

...
auc_tab_df = left_join(auc_tab_df, train_hu_anno[,c('var_id','UniProt_acc')], by=c('Antibody'='var_id'))
...
for (ii in c(1:9)) {
  ...
  p <- ggplot(subgroup, aes(x = Gene, y = AUC)) +
    ...
    theme(plot.title = element_text(face = "bold", size = 14), axis.title  = element_text(size = 12), axis.text   = element_text(size = 12))
...
run_lasso_feature_analysis <- function(
  ...
  p_coef <- ggplot(keys3, aes(x = reorder(variables, -abs(beta)), y = beta, fill = product)) +
    ...
    geom_text(aes(label = sig_marker,
              ...
              size = 3) +
    labs(x = paste(virus_short, "peptides"), y = "LASSO weight", fill = "Product") +
    theme_minimal() +
  ...
  ragg::agg_png(file.path(out_dir, paste0("figure4_", virus_short, "_", antibody_id, "_weights.png")),
                width = 2200, height = 1100, res = 300)
  ...
  results_abc1 <- left_join(results_abc, train_vi_anno[, c("UniProt_acc","product")],
